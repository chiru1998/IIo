...
#VARIABLE USED FOR ALARM DISABLING
...
connection=sqlite3.connect(config.DATABASENAME,check_same_thread=False)
...
#new function defined for inserting login details to localdb
def insertoperatorinouttime(EmpId,MachineId,inTime,currenttime,outTime,isActive):
    sql="INSERT INTO OperatorInOutTime(emp_id,machineId,inTime,inDate,outTime,active) VALUES(?,?,?,?,?,?)"
    values=(EmpId,MachineId,inTime,currenttime,outTime,isActive)
    try:
        if(cursor.execute(sql,values)):
            connection.commit()
            print("Login Details Posted Successfully")
    except Exception as e:
        print(e)
        print("failed to insert operatorinouttime details")
        
#new function defined for updating logout details to localdb
def updateoperatorinouttime(endTime,isActive,EmpId):
    try:
        sql="update OperatorInOutTime set outTime = ?,active =? where emp_id=?"
        values=(endTime,isActive,EmpId)
        cursor.execute(sql,values)
        connection.commit()
        print("updated operatorinouttime details  ")
    except  Exception as e:
        print(e)
        print("failed to update operatorinouttime details")
        
#function to update date and time      
def updateoperatorinoutdate(starttime,startdate,empid):
    try:
        sql="update OperatorInOutTime set inTime = ?,inDate = ?  where emp_id=?"
        values=(starttime,startdate,empid)
        cursor.execute(sql,values)
        connection.commit()
        print("updated operatorinoutdate details  ")
    except  Exception as e:
        print(e)

#function returning the empid count
def fetchoperatorcount(empid):
    try:
        sql="select count(emp_id) from OperatorInOutTime where emp_id = ?"
        values=(empid,)
        cursor.execute(sql,values)
        v=cursor.fetchone()
        print(v)
...
#new function defined for fetching login details from localdb     
def fetchoperatorinouttime(col,empid):
    try:        
        sql="select * from OperatorInOutTime where emp_id=?"
        values=(empid,)
        cursor.execute(sql,values)
...
#new function defined for updating maintenance status to localdb
def updatemaintenancestatus(maintenancestatus):
    try:
        data=cursor.execute("SELECT MAX(id) FROM live_status")
        if(data!= None):
            lastId=cursor.fetchone()[0]
            sql="update live_status set maintenanceStatus=? where id=?"
            values=(maintenancestatus,lastId)
            cursor.execute(sql,values)
            connection.commit()
            print("updated maintenance details")
    except  Exception as e:

        print(e)
        print("failed to update maintenance details")
...
def returnCurrentSignal():
  ...
  try:
      if isAlarmDisabled == False:
        ...
        #to display "maintenance mode" on react screen
        result=liveStatus.query.get(1)
  except Exception as e:
  ...
  print(presentDate)
  print("IsAlarmDisabled : ",isAlarmDisabled)
  print("```````Test Live signal`````` ",liveSignal)
  try:  
...
def login():
   global EmpId,OperatorName,startTime,isActive,endTime,machineId,CurrentDate
   ...
   try:
         ...
         if(data['Error']!=None):
              ...
              resultData['EmpCode']=data['EmpCode']
              ...
              #Variable created to fetch empid
              employee=fetchoperatorcount(EmpId)
              #check employee id if exist or not
              if (employee > 0):
                  isActive = True
                  startTime =datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                  logintime=fetchoperatorinouttime(3,EmpId)
                  startDate = datetime.now().strftime("%Y-%m-%d")
                  Startdate=fetchoperatorinouttime(4,EmpId)
                  #updating start time to end time
                  updateoperatorinouttime(startTime,isActive,EmpId)
                  #compare current date and shift timings
                  if (Startdate != startDate) and ("05:00:00"<=startTime<="23:59:00"):
                      updateoperatorinoutdate(startTime,startDate,EmpId)
                      logintime=startTime
                  postOperatorLoginEndpointData = {
                      "empID" : EmpId,
                      "startTime" : logintime,
                 ...
                 endTime =startTime                      
                 insertoperatorinouttime(EmpId,machineId,startTime,CurrentDate,endTime,isActive)                  
                 postOperatorLoginEndpointData = {
                      "empID" : EmpId,
                      "startTime" : startTime,
                      "endTime" : endTime,
       ...
       return jsonify({"result": {"status":1,"admin":False,"message":"Something Went Wrong, Check Network Connection"}})
...
# shift data hard corded to logic is commented
       ...
       for data in datas: 
          ...
          try:
             ...
             if(result!=None):
                ...
                db.session.commit() 
                print("added shift data to database") 
          except Exception as e:
             print(e)
...
def operatorScreen():
    ...
    productionObj=production(operatorName=username,jobId=jobId,shift=shift,component=component,modelName=model,operation=operation,cycleTime="5.5",inspectionStatus="0",status="0",timeStamp=var_time,machineId=machineId,date=presentDate,progress="not started",emp_id=EmpId,inspectionCheck="0",type = "Add",inspectionTimeStamp=var_time)
...
def Logout():
   ...
   headers = config.HEADERS 
   endTime = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
   #updating logout details to localdb
   updateoperatorinouttime(endTime,isActive,EmpId)
   logintime=fetchoperatorinouttime(3,EmpId)
   postOperatorLoginEndpointData = {
    "empID" : EmpId,
    "startTime" : logintime,
...
def PostOperationLoginDetails():
    ...
    postOperatorLoginEndpoint = "http://" + config.SERVER_IP + config.SERVER_ENDPOINT_START + "/PostOperatorLoginDetails"
    headers = config.HEADERS
    #To fetch the start time from localDB
    startTime=fetchoperatorinouttime(3,EmpId)
    endTime = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    postOperatorLoginEndpointData = {
    "empID" : EmpId,
    "startTime" : startTime,
...
def maintenance():
    ...
    #updating maintenance on condition to localdb
    updatemaintenancestatus("maintenanceOn")
    return ("",204)
    
#alarm screen  enabling under maintenance mode
@app.route('/enableMachineUnrelease', methods=['POST'])
def enableMachineUnrelease():
    global isAlarmDisabled
    result = request.get_json()
    GPIO.output(holdingPin,False)
    isAlarmDisabled = False
    #updating maintenance off condition to localdb
    updatemaintenancestatus("maintenanceOff")
    return jsonify({"result": {"status":1,"message":"successfully data saved"}})    
...
@app.route('/updateNetworkDehbaordtails', methods = ['POST'])
